...
import { findNode, parse } from "./parser";
...
export function activate(context: ExtensionContext) {
	...
	context.subscriptions.push(
		languages.registerCompletionItemProvider(
			"azcli",
			new AzCompletionItemProvider(azService),
			" ",
		),
	);
	context.subscriptions.push(
	...
	provideCompletionItems(
		...
		return this.azService
			.getCompletions(
				subcommand[0] === "az"
					? {
							subcommand: subcommand.slice(1).join(" "),
							argument,
							arguments: args,
						}
					: {},
...
class RunLineInEditor {
		...
		if (command.length > 0) {
			...
			if (this.runningCommandCount === 1) {
				...
				this.statusBarUpdateInterval = setInterval(() => {
					if (this.runningCommandCount === 1) {
						this.commandRunningStatusBarItem.text = `${
			...
			return this.findResultDocument()
				.then((document) =>
					window.showTextDocument(document, ViewColumn.Two, true),
				)
				.then((target) =>
					replaceContent(
						target,
						JSON.stringify({
							[l10n.t("Running command")]: command,
						}) + "\n",
					)
						.then(() => exec(command))
						.then(
	...
	private getSelectedCommand(source: TextEditor) {
		...
		if (source.selection.isEmpty) {
			...
			var command = this.stripComments(
				source.document.lineAt(lineNumber).text,
			);
			...
			if (selectionStart.line === selectionEnd.line) {
				...
				for (
					...
					var line = this.stripComments(
						source.document.lineAt(index).text,
					);
	...
	private commandFinished(startTime: number) {
		...
		if (this.runningCommandCount === 0) {
			...
			this.hideStatusBarItemTimeout = setTimeout(
				() => this.commandRunningStatusBarItem.hide(),
				10000,
			);
		}
	}
